...
def build_ingest_summary(
    *,
    ...
    config = official_energy_history_provider.load_official_energy_history_config(data_sources)
    ...
    fetched = official_energy_history_provider.fetch_official_energy_history(
        config,
        limit=limit,
        fetcher=fetcher,
    )
    ...
    normalized = official_energy_history_provider.normalize_official_energy_history(
    ...
    observations = official_energy_history_provider.build_market_observations(records)
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Dry-run or ingest official FRED/EIA WTI and Brent history into market_history."
    )
    ...
    parser.add_argument("--limit", type=int, default=370, help="FRED observation limit per series.")
