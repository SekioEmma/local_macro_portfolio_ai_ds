...
def build_ingest_summary(
    *,
    ...
    if result.get("status") != "ok":
        ...
        return summary
    buckets = gdelt_client.normalize_story_summary(
        result["payload"],
        query_key=query_key,
        window_start=window_start,
        window_end=window_end,
    ...
    for bucket in buckets:
        if dry_run:
            continue
        research_context_store.upsert_story_bucket(bucket, db_path=db_path)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Ingest aggregate GDELT Cloud story summary buckets."
    )
    parser.add_argument("--preset", required=True, choices=sorted(STORY_PRESETS))
