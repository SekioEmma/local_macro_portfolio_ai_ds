...
| 详细 holdings consent / injection contract 已实现，production snapshot provider 未接线时 activation fail-closed | `GET /api/agent/capabilities` reports `holdings_snapshot_backend_not_wired`; consent endpoint does not issue tokens while unwired; `tests/ai/test_holdings_consent_service.py`; `tests/ai/test_holdings_external_context_service.py`; `tests/api/test_agent_run_route.py`; `tests/api/test_agent_stream_route.py`; `app_frontend/src/api/client.test.ts`; `app_frontend/src/components/AIChatPage.test.tsx` | guarded foundation |
...
| Temporal Alignment Gate 已实现 | `src/app_backend/services/temporal_alignment_service.py`; `tests/ai/test_temporal_alignment_service.py`; `tests/api/test_agent_run_route.py` and `tests/api/test_agent_stream_route.py` reject client-supplied `current_date`; controlled run reports expose `asynchronous_inputs` and cutoffs | implemented |
| SSE 已实现 | `src/app_backend/services/agent_stream_service.py`; `tests/api/test_agent_stream_route.py` covers lifecycle, cancel, queue overflow, holdings non-leakage, and unhandled exception sanitization; `app_frontend/src/components/AIChatPage.tsx`; `app_frontend/src/components/AIChatPage.test.tsx` | implemented |
...
| RAG generation contract 已实现 | `src/app_backend/services/rag_index_generation.py`; `src/app_backend/services/curated_rag_ingest.py`; `src/llm/vector_store.py`; `scripts/validate_local_rag.py`; `tests/ai/test_validate_local_rag_script.py`; `tests/ai/test_curated_rag_ingest.py`; `tests/llm/test_vector_store.py`; `docs/ADR/ADR-0006-rag-index-generation-and-embedding-compatibility.md` | implemented |
| 所有关键测试通过 | Latest local gate: `python -m pytest -q tests/ai tests/api tests/contracts tests/llm` = `2860 passed, 14 skipped`; `python -m ruff check src tests scripts` passed; frontend typecheck/test/build = `21 passed` and build passed | verified locally |
