...
| 详细 holdings consent 已实现并通过安全测试 | `tests/ai/test_holdings_consent_service.py`; `tests/ai/test_holdings_external_context_service.py`; `tests/api/test_agent_run_route.py`; `tests/api/test_agent_stream_route.py`; `app_frontend/src/api/client.test.ts`; `app_frontend/src/components/AIChatPage.test.tsx` | implemented |
...
| 所有关键测试通过 | Latest local gate: `python -m pytest -q tests/ai tests/api tests/contracts tests/llm` = `2855 passed, 14 skipped`; `python -m ruff check src tests scripts` passed; frontend typecheck/test/build = `19 passed` and build passed | verified locally |
