...
| Claim-Evidence Ledger 已实现 | `src/app_backend/services/run_evidence_ledger.py` stores atomic observations; `src/app_backend/services/claim_evidence_validator.py` binds observed/reported fact `value` / `unit` / `as_of` to current-run ledger observations; `tests/ai/test_run_evidence_ledger.py`; `tests/ai/test_claim_evidence_validator.py`; `tests/ai/test_agent_evidence_ledger_registration.py`; `tests/ai/test_agent_runtime_mocked.py` | implemented |
...
| SSE 已实现 | `src/app_backend/services/agent_stream_service.py` emits sanitized lifecycle, heartbeat, cancellation, conflict, and brief-section events; `src/app_backend/services/agent_run_registry.py` enforces one active stream lease per session and clears it in worker cleanup; client disconnects request cancellation through `AgentRunRegistry`; `tests/ai/test_agent_run_registry.py`; `tests/api/test_agent_stream_route.py` covers lifecycle, explicit cancel, duplicate session conflict, lease release, disconnect cancel, heartbeat, queue overflow, holdings non-leakage, and unhandled exception sanitization; `app_frontend/src/components/AIChatPage.tsx`; `app_frontend/src/components/AIChatPage.test.tsx` | implemented |
...
- Curated RAG ingest preflights existing nonempty index compatibility before
  document preparation, chunk writes, vector deletes, or vector upserts.
...
## F-Evidence Numeric Binding Refresh

- `reported` facts with structured `value` / `unit` / `as_of` must bind to a
  matching atomic observation on the cited evidence.
- `reported` narrative facts may remain structured-null when no atomic
  observation exists.
- `unavailable` facts must keep `value`, `unit`, and `as_of` null.
- Institutional MEMO material can support interpretation or reported narrative,
  but it cannot silently become an observed numeric value.
...
- Detailed holdings UI/API remains disabled/fail-closed until a Phase G+ real provider and disclosure policy are implemented and accepted.
