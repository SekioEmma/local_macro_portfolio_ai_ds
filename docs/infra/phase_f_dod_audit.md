...
| Trace 长期保存已实现 | `src/app_backend/services/agent_trace_service.py`; `tests/ai/test_agent_trace_service.py`; `docs/ADR/ADR-0005-phase-f-sse-and-trace-retention.md` | implemented |
| RAG generation contract 已实现 | `src/app_backend/services/rag_index_generation.py`; `scripts/validate_local_rag.py`; `tests/ai/test_validate_local_rag_script.py`; `docs/ADR/ADR-0006-rag-index-generation-and-embedding-compatibility.md` | implemented |
| 所有关键测试通过 | Latest local gate: `python -m pytest -q tests/ai tests/api tests/contracts` = `2739 passed, 14 skipped`; `python -m ruff check src tests scripts` passed; frontend typecheck/test/build passed | verified locally |
...
- Required MacroBrief product label remains: `研究辅助输出` / `非自动投资决策` / `需要用户审阅`.
