...
- Phase F runtime timeout checkpoint：agent runtime 已加入 wall-clock / provider-call / tool-call timeout budget；默认 DeepSeek transport 复用 provider-call timeout；SSE bridge 已加入 bounded queue 与 sanitized queue-overflow error，避免慢客户端或事件洪峰造成无界内存增长。
