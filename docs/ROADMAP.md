...
| C | 搜索分类持久化 + 经济日历 | 进行中：C1 已完成；C2 official historical data ingest 已完成；C3 guarded local knowledge base store 已完成；C4a offline calendar foundation 已完成；C4d read path hardening 已完成；C4b guarded BLS/BEA manual acquisition 已完成；C4c acquisition result boundary hardening 已完成；FOMC exact-time deferred | — |
| D | RAG 知识库 | 2 | — |
| E | 情景化收益区间引擎 | 2 | ✅ E1 |
| F | Agent + 9 节前端 | 2.5 | — |
| G | 报告归档 + 历史对比 | 1 | — |
