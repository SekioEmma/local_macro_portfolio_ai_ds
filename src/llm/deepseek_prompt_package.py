    ...
    *,
    ...
    user_payload = {
        ...
        "reasoning_requirements": [
            ...
            "数据证据表必须新增一列：metric_key（必须是 data_package 中的 exact metric_key）。",
            "正文引用任何数值时，证据表必须单独列出对应 metric_key，不得只写在解释列。",
            ...
            "正文提到指标时应使用 data_package 的 exact metric_key，不要添加 data_package 未提供的口径词（如“非季调”）。",
        ],
        "required_sections": [
            "正文必须包含独立章节标题：当前数据缺失与不可判断事项。",
            "该章节不得省略，且必须集中列出以下缺失边界：缺失 FedWatch probability，无法量化降息/加息概率；缺失 PE / forward PE / CAPE / FactSet / Bloomberg / Reuters 估值数据，无法断言估值倍数或估值百分位；缺失 consensus CPI/PPI，不能写“超预期/低于预期”；缺失 intraday Treasury high，FRED DGS10/DGS30 只能作为日度 constant maturity yield，不是盘中高点；未提供 PPI final demand，PPIACO 只能作为 all commodities PPI，不得写成 final demand PPI；缺失 market breadth / mega-cap concentration / equal-weight vs cap-weight 数据，无法确认市场广度或集中度恶化；油价数据若 freshness=stale，只能写“截至 observation_date 的数据”，无法确认当前实时油价。",
            "即使这些缺失项已在证据表或 data limitations 出现，也必须在该章节集中列出。",
            "memo 在结尾前必须包含该章节、DCA 与现金准备金边界、免责声明，避免因长篇展开导致尾部截断。",
        ],
        ...
        "rates_inflation_oil_rules": [
            ...
            "不得使用“突破5%”“确认突破”“站上5%”等话术；只能写“按本项目定义满足 5%阈值确认条件”或“latest daily FRED observation is above 5%”，并必须说明这不是盘中高点（本地数据未提供盘中高点）。",
